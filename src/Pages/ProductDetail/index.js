  ...
  const addToFavourites = () => {
    dispatch({
      ...
      item: {
        id: product.id,
        title: product.title,
        image: product.images[0], // Use the first image as the product image in favourites
        price: product.price,
        rating: product.rating,
        mrp: product.mrp,
    ...
    <div className="imagePreviews">
        {product.images.map((img, index) => (
          <img
            ...
            onMouseClick={() => handleImageClick(index)}
          />
        ))}
      </div>
      <div className="productDetail_imageContainer">
        <button className="imageNavButton" onClick={handlePrevImage}>{"<"}</button>
        <img src={product.images[currentImageIndex]} alt={product.title} className="productDetail_image" />
        <button className="imageNavButton" onClick={handleNextImage}>{">"}</button>
      </div>
      ...
      <div className="productDetail_info">
        ...
        <p className="productDetail_price">
          ...
          <strong style={{ textDecoration: 'line-through', color: 'grey', fontWeight: 'normal', marginLeft: '10px',fontSize:'18px' }}>{product.price}</strong>
        ...
        <div className="productDetail_buttons">
          ...
          <div className="productDetail_quantityControl">
            ...
            <span style={{marginLeft:'15px', marginRight:'15px', fontWeight:'bold'}}>{quantity}</span>
