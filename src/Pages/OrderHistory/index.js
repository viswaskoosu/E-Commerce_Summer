...
function OrderHistory() {
  ...
  const filterOrders = (orders, filter) => {
    ...
    switch (filter) {
      case '3months':
        filteredOrders = orders.filter(order => (now - new Date(order.date)) <= 90 * 24 * 60 * 60 * 1000);
        break;
      case '6months':
        filteredOrders = orders.filter(order => (now - new Date(order.date)) <= 180 * 24 * 60 * 60 * 1000);
        break;
      case '1year':
        filteredOrders = orders.filter(order => (now - new Date(order.date)) <= 365 * 24 * 60 * 60 * 1000);
        break;
      case '2years':
        filteredOrders = orders.filter(order => (now - new Date(order.date)) <= 2 * 365 * 24 * 60 * 60 * 1000);
        break;
      case '5years':
        filteredOrders = orders.filter(order => (now - new Date(order.date)) <= 5 * 365 * 24 * 60 * 60 * 1000);
        break;
      default:
              ...
              <div key={item.id} className="orderHistory_item">
                ...
                <div className="orderHistory_info">
                  ...
                  <div className="orderHistory_itemActions">
                    <button onClick={() => buyItAgain(item)}>Buy it again</button>
